...
impl RedisRateLimiter {
    ...
    pub async fn check_auth_rate_limit(&self, ip: &str, username: Option<&str>) -> Result<(bool, AuthRateLimitInfo)> {
        ...
        if is_blocked.is_some() {
            return Ok((false, AuthRateLimitInfo {
                allowed: false,
        ...
        if ip_exceeded || user_exceeded {
            ...
            let reason = if ip_exceeded && user_exceeded {
                format!("Too many login attempts from this IP ({}/{}) and for this user ({}/{})", 
                    ip_count, self.auth_ip_limit, user_count, self.auth_user_limit)
            } else if ip_exceeded {
                format!("Too many login attempts from this IP ({}/{})", ip_count, self.auth_ip_limit)
            } else {
                format!("Too many login attempts for this user ({}/{})", user_count, self.auth_user_limit)
            };
    ...
    pub async fn block_ip(&self, ip: &str, reason: &str, permanent: bool) -> Result<()> {
        ...
        tracing::warn!("IP {} blocked. Reason: {}. Attempts: {}. Permanent: {}", 
            ip, reason, attempt_count, permanent || self.ip_block_duration_hours == 0);
